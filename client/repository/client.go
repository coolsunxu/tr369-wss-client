	...
	for {
		...
		case <-repo.pingTicker.C:
			...
			if repo.writeCount == 0 {
				logger.Infof("No data change detected, skipping save.")
				continue
			}
			// 保存数据
			...
			// 重置最后写入时间
			repo.lastWriteTime = time.Now().UnixMilli()
			logger.Infof("tick save data synchronized.")
		case <-repo.ctx.Done():
			return
		}
	}
...
func (repo *clientRepository) SaveData() {
	...
	if repo.writeCount >= repo.Config.DataRefreshConfig.WriteCountThreshold {
		...
		logger.Infof("normal save data synchronized.")
	}

	logger.Infof("current write count: %d", repo.writeCount)
